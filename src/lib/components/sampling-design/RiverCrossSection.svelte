	...
	<div class="relative h-80 col-span-2">
		...
		>
			...
			<select
				...
				bind:value={$samplingDesign.riverCrossSection.bank}
			>
		...
		<label
			class={clsx(
				...
				$samplingDesign.riverCrossSection.surface ? 'w-36 left-[15rem]' : 'w-10 left-[19rem]'
			)}
		>
			Surface
			<select
				class="text-sm py-1 px-2 bg-white w-full rounded-md border border-description"
				bind:value={$samplingDesign.riverCrossSection.surface}
			>
				<option value="" disabled />
		...
		<label class="flex items-center gap-3 absolute -top-10 right-10">
			<input type="checkbox" bind:checked={$samplingDesign.riverCrossSection.riparianZone} />
		...
		<label
			class={clsx(
				...
				$samplingDesign.riverCrossSection.bed ? 'w-44 left-[15rem]' : 'w-10 left-[19rem]'
			)}
		>
			Bed
			<select
				bind:value={$samplingDesign.riverCrossSection.bed}
				class="text-sm py-1 px-2 bg-white w-full rounded-md border border-description"
			>
				<option value="" disabled />
