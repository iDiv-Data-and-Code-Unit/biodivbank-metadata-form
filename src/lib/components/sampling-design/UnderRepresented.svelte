<script lang="ts">
	...
	function removeGroup(ur: { group: string; reason: string }) {
		samplingDesign.update((sd) => {
			sd.samplingScope.underRepresentedTaxa = sd.samplingScope.underRepresentedTaxa.filter(
				(u) => u !== ur
			);
			return sd;
		});
...
<Question
	...
	direction="column"
>
	{#if $samplingDesign.samplingScope.underRepresentedTaxa.length}
		<div class="col-span-2 space-y-1">
			{#each $samplingDesign.samplingScope.underRepresentedTaxa as ur}
				<div
				...
				>
					...
					<span class="col-span-4 text-ellipsis overflow-clip whitespace-nowrap">{ur.reason}</span>
